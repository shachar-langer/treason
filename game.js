...
module.exports = function createGame(options) {
  ...
  function playerJoined(playerIface) {
    ...
    if (countReadyPlayers() < state.maxPlayers) {
      ...
      countReadyPlayers(true) < state.maxPlayers &&
      makeAisObservers()
    ) {
      isObserver = false
    } else {
  ...
  function playerLeft(playerIdx) {
      ...
      if (!playerState.isObserver) {
        ...
        for (var j = 0; j < influence.length; j++) {
          if (!influence[j].revealed) {
            historySuffix.push(
              format('{%d} revealed %s', playerIdx, influence[j].role)
            )
        ...
        if (!end) {
          if (state.state.playerIdx == playerIdx) {
            ...
            state.state.name == stateNames.REVEAL_INFLUENCE &&
            state.state.playerToReveal == playerIdx
          ) {
            nextTurn()
          } else if (
    ...
    addHistory(
      'player-left',
      nextAdhocHistGroup(),
      playerState.name + ' left the game'
    )
  ...
  function playerReady(playerIndex) {
    ...
    if (!playerState.isReady) {
      ...
      addHistory(
        'player-ready',
        nextAdhocHistGroup(),
        playerState.name + ' is ready to play again'
      )
    }
  }
  ...
  function emitState(emitStateChangeEvent) {
    if (
      state.state.name === stateNames.WAITING_FOR_PLAYERS ||
  ...
  function start(gameType) {
    ...
    for (let i = 0; i < state.numPlayers; i++) {
      ...
      if (playerState.isReady !== true) {
        // it could also be false or 'observe'
        if (playerState.ai) {
    ...
    state.numRoles =
      3 +
      (nonObservers.length > 6 ? Math.floor((nonObservers.length - 5) / 2) : 0)
    deck = buildDeck()
  ...
  function countReadyPlayers(skipAi) {
    ...
    for (var i = 0; i < state.numPlayers; i++) {
      var playerState = state.players[i]
      if (playerState.isReady === true && (!skipAi || !playerState.ai)) {
        // it could also equal 'observe'
        readyCount++
      }
    }
    return readyCount
  ...
  function command(playerIdx, command) {
    ...
    if (command.command == 'leave') {
      ...
      throw new GameException(
        'Stale state (' + command.stateId + '!=' + state.stateId + ')'
      )
      ...
      if (action.targeted) {
        if (command.target == null) {
          throw new GameException(
            'No target specified for action ' + command.action
          )
        }
        ...
        if (
          ...
          !canTarget(playerIdx, command.target)
        ) {
      ...
      if (state.state.name == stateNames.ACTION_RESPONSE) {
        ...
        if (
          !state.allowChallengeTeamMates &&
          !canTarget(playerIdx, state.state.playerIdx)
        ) {
        ...
        if (
          !state.allowChallengeTeamMates &&
          !canTarget(playerIdx, state.state.target)
        ) {
          throw new GameException('Cannot challenge player on the same team')
        }
      ...
      if (
        state.state.name != stateNames.ACTION_RESPONSE &&
        state.state.name != stateNames.FINAL_ACTION_RESPONSE
      ) {
        throw new GameException('Incorrect state')
      }
      action = actions[state.state.action]
      ...
      if (command.forceExchange) {
        ...
        addHistory(
          'interrogate',
          curTurnHistGroup(),
          '{%d} allowed {%d} to keep the same roles',
          playerIdx,
          state.state.target
        )
      }
  ...
  function canTarget(playerIdx, target) {
    return (
      state.freeForAll ||
      state.players[playerIdx].team != state.players[target].team
    )
  }
  ...
  function allow(playerIdx) {
    if (state.state.name == stateNames.BLOCK_RESPONSE) {
      ...
      if (state.state.name == stateNames.FINAL_ACTION_RESPONSE) {
        if (state.state.target != playerIdx) {
          throw new GameException(
            'Only the targetted player can allow the action'
          )
        }
      } else {
        allows[playerIdx] = true
  ...
  function challenge(playerIdx, challengedPlayerIdx, challengedRole) {
    ...
    if (proof != null) {
      ...
      var message = format(
        '{%d} incorrectly challenged {%d}; {%d} exchanged %s for %s',
        playerIdx,
        challengedPlayerIdx,
        challengedPlayerIdx,
        oldRoles,
  ...
  function playAction(playerIdx, actionState) {
    ...
    if (actionState.action == 'assassinate') {
      ...
      addHistory(
        'change-team',
        curTurnHistGroup(),
        '{%d} changed to the %s team',
        playerIdx,
        getTeamName(playerState.team)
      )
      ...
      addHistory(
        'convert',
        curTurnHistGroup(),
        '{%d} converted {%d} to the %s team',
        playerIdx,
        actionState.target,
        getTeamName(target.team)
      )
      ...
      addHistory(
        'convert',
        curTurnHistGroup(),
        '{%d} embezzled $%d from the treasury',
        playerIdx,
  ...
  function indexOfInfluence(playerState, role) {
    for (var i = 0; i < playerState.influence.length; i++) {
      if (
        playerState.influence[i].role == role &&
        !playerState.influence[i].revealed
      ) {
        return i
      }
    }
