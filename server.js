...
var argv = require('optimist')
  .usage(
    '$0 [--debug] [--recreate-views] [--port <port>] [--log <logfile>] [--db <database>]'
  )
  .default('port', 8080)
  .default('log', 'treason.log')
  .default('db', 'treason_db').argv
...
io.on('connection', function (socket) {
  ...
  socket.on('registerplayer', function (data) {
    ...
    dataAccess
      .register(data.playerId, data.playerName, userAgent)
      .then(function (playerId) {
        socket.playerId = playerId

        var playerName = data.playerName
        var currentOnlinePlayers = filterPlayers().concat([
...
function isInvalidPlayerName(playerName) {
  return (
    ...
    !playerName.match(/^[a-zA-Z0-9_ !@#$*]+$/ || !playerName.trim())
  )
}
